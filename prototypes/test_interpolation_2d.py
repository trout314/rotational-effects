...
# Allow running as either `python prototypes/test_interpolation_2d.py` from
# the repo root or `python test_interpolation_2d.py` from within prototypes/.
# Either way, resolve `data/` and the sibling `interpolation_2d` module
...
from prototypes.interpolation_2d import (  # noqa: E402
...
potential_files_and_angles = [
    (os.path.join(_DATA_DIR, f"PC.in.{a:03d}"), a) for a in range(0, 181, 10)]
long_range_pow = -4
