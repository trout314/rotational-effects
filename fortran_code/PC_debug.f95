MODULE GLOBAL
  !This is the data module for program PC.
  IMPLICIT NONE
  ...
  INTEGER,PARAMETER::MAXE=65,MAXL=30,INOOMAX=40,MAXORBITS=600
  ...
  INTEGER::NMAXCH,NPTSCH,NV,NLONG,NO,INOO,NAITK

  !Declare the global integer array.
  ...
  REAL(SELECTED_REAL_KIND(18))::HPI,PI,ACCURACY,EMIN,EMAX,ACC1,EXPON,&
    CSHORT,CLONG,ED,EC,EC2,EE,ROMAX,BOMAX,RM1,THO,EING,BING,R1ING,R2ING,&
    EAING,EBING
  ...
  REAL(SELECTED_REAL_KIND(18)),DIMENSION(:),ALLOCATABLE::CHEPTS,CHEWTS,&
    CPOTEN
  REAL(SELECTED_REAL_KIND(18)),DIMENSION(:,:),ALLOCATABLE::POTEN
...
PROGRAM PC 
  !This program was written in 2010 by Larry A. Viehland, Department
  !of Science, Chatham University, Pittsburgh, Pennsylvania 15232 USA.
  !It computes the transport cross sections for atomic ions interact-
  !ing with atomic ions, using the techniques in L. A. Viehland and
  !Y. Chang, Comp. Phys. Comm. 181 (2010) 1687-1696.  It was modified
  !in June, 2011.
  USE GLOBAL
  IMPLICIT NONE
  ...
  IF(ERROR/=0)THEN
    PRINT *,'Could not open the old file named ',CCFILE(1:LENGTH),'.'
  ...
  DO I=1,NPTSCH
    ...
    IF(ERROR/=0)THEN
      PRINT *,'File ',CCFILE(1:LENGTH),' is incompatible with this &
        &program.'
  ...
  !Call subroutine INPUT to read the potential information, subroutine
  !ORBITS to determine the orbiting parameters, subroutine REGIONS to
  !determine the (possibly multiple) orbiting regions, and subroutine
  !EFIT to calculate and print the cross sections.
  CALL INPUT
  CALL ORBITS
  CALL REGIONS
  CALL EFIT
...
SUBROUTINE INPUT
  ...
  !Declare the local character variable.
  ...
  !Open the files.
  OPEN(UNIT=12,FILE='PC.in',STATUS='OLD',IOSTAT=ERROR)
  IF(ERROR/=0)THEN
    PRINT *,'Could not open the old file named PC.in'
    STOP
  ENDIF
  OPEN(UNIT=13,FILE='PC.out',IOSTAT=ERROR)
  IF(ERROR/=0)THEN
    PRINT *,'Could not open a file named PC.out'
    STOP
  ENDIF

  !Read in COMMENT, ACCURACY, EMIN, EMAX and NV.
  READ(12,'(A)',IOSTAT=ERROR)COMMENT
  IF(ERROR/=0)THEN
    WRITE(13,*)'Something is wrong with line 1 of file PC.in'
    STOP
  ENDIF
  READ(12,*,IOSTAT=ERROR)ACCURACY,EMIN,EMAX
  IF(ERROR/=0)THEN
    WRITE(13,*)'Something is wrong with line 2 of file PC.in'
    STOP
  ENDIF
  READ(12,*,IOSTAT=ERROR)NV
  IF(ERROR/=0)THEN
    WRITE(13,*)'Something is wrong with line 3 of file PC.in'
    STOP
  ENDIF

  !Check NV for a value that is too low.
  IF(NV<=3)THEN
    WRITE(13,*)'The NV parameter on line 3 of file PC.in must be &
      &4 or more.'
    STOP
  ENDIF
  ...
  !Read in the value for NLONG and check for appropriate values.
  READ(12,*,IOSTAT=ERROR)NLONG
  IF(ERROR/=0)THEN
    WRITE(13,*)'Something is wrong with line 4 of file PC.in'
    STOP
  ENDIF
  IF(NLONG/=3.AND.NLONG/=4.AND.NLONG/=6)THEN
    WRITE(13,*)'At present, this program will work only for long-range'
    WRITE(13,*)'potentials that vary as the inverse of the separation'
    WRITE(13,*)'to the third, fourth or sixth power.'
    WRITE(13,*)'Check line 4 on file PC.in.'
    STOP
  ENDIF
  ...
  READ(12,*,IOSTAT=ERROR)POTEN(1,1),POTEN(2,1)
  IF(ERROR/=0)THEN
    WRITE(13,*)'Error reading the first tabulated potential point &
      &from file PC.in'
    STOP
  ENDIF

  !Make sure that EMAX is not greater than the first potential value.
  EMAX=MIN(EMAX,POTEN(2,1))

  !Read in the rest of the points, making sure that R increases.
  DO I=2,NV
    READ(12,*,IOSTAT=ERROR)POTEN(1,I),POTEN(2,I)
    IF(ERROR/=0)THEN
      WRITE(13,'(A,I3,A)')'Error reading the ',I,'-th tabulated &
        &potential from file PC.in'
      STOP
    ELSEIF(POTEN(1,I)<=POTEN(1,I-1))THEN
      WRITE(13,*)'The separations do not increase monotonically.'
      WRITE(13,'(A,F12.8)')'Check file PC.in at separation ',&
        &POTEN(1,I)
      STOP
    ENDIF
  ENDDO
  ...
  DO I=1,3
    IF(POTEN(2,I)<ZERO)THEN
      WRITE(13,'(A,I1,A)')'Tabulated potential value ',I,' is negative.'
      WRITE(13,*)'This is not allowed, so add more points at smaller'
      WRITE(13,*)'separation before running this program again.'
      STOP
    ENDIF
  ENDDO
  ...
  IF(NLONG==4.OR.NLONG==6)THEN
    DO I=NV-2,NV
      IF(POTEN(2,I)>ZERO)THEN
        WRITE(13,'(A,I4,A,I1)')'Tabulated potential value ',I,' is &
          &positive and NLONG=',NLONG
        WRITE(13,*)'This is not allowed, so add more points at larger'
        WRITE(13,*)'separation before running this program again.'
        STOP
      ENDIF
    ENDDO
  ...
  I=INDEX(COMMENT//'  ','  ')
  WRITE(13,'(A,/,A)')'Transport cross sections for ',COMMENT(1:I-1)
...
SUBROUTINE ORBITS
  ...
  DO
    ...
    IF(C1>ZERO.AND.C2<ZERO.AND.VEFF>ZERO)THEN
      ...
      NO=NO-1
      IF(NO<=0)THEN
        WRITE(13,*)' More room is needed for orbiting parameters!'
        STOP
      ENDIF
      ORBIT(1,NO)=VEFF
      ...
      !of accuracy, checking to see if this is an energy where one
    ...
    IF(R>1.0D3)THEN
      IF(NO>MAXORBITS)THEN
        WRITE(13,*)'Even at 1000 bohr, the potential did not support &
          &orbiting.'
        STOP
      ENDIF
      EXIT
  ...
  DO
    ...
    IF(J>NO-19)THEN
      WRITE(13,'(A)')'There is not enough room for the orbiting &
        &parameters.'
      STOP
    ENDIF
  ...
  DO J=2,NO
    ...
    IF(INCREASING.AND.ORBIT(1,J)<ORBIT(1,J-1))THEN
      ...
      IF(INOO>INOOMAX)THEN
        WRITE(13,*)'INOOMAX is too small.  You must reset this named'
        WRITE(13,*)'constant, recompile the program and try again.'
        STOP
      ENDIF
      INCREASING=.FALSE.
...
RECURSIVE SUBROUTINE INTEG(LMAX,F,A,B,EA,EB,ANS,ACC,IPR,PRANS)
  ...
  IF(LMAX<1)THEN
    WRITE(13,*)'ERROR!  INTEG was called with LMAX=',LMAX
    STOP
  ENDIF
...
SUBROUTINE EFIT
  ...
  !Declare the local integer variables.
  !  ERROR allows for recovery from errors in opening the files.
  ...
  INTEGER::ERROR,NQEC,INDEX,NSTAR,INM,NM,I,J,L,NPRINT,JSTRT
  ...
  !    coefficients.
  REAL(SELECTED_REAL_KIND(18))::E1,E2,E,E2P1,E2M1,XCHEB,TJ,ACHEB,CCHEB
  ...
  OPEN(16,FILE='crosec',ACCESS='DIRECT',IOSTAT=ERROR,RECL=496)
  IF(ERROR/=0)THEN
    WRITE(13,*)'Error opening file crosec.'
    STOP
  ENDIF
  OPEN(17,FILE='coeffs',ACCESS='DIRECT',IOSTAT=ERROR,RECL=480)
  IF(ERROR/=0)THEN
    WRITE(13,*)'Error opening file coeffs.'
    STOP
  ENDIF
  OPEN(18,FILE='ninval',IOSTAT=ERROR)
  IF(ERROR/=0)THEN
    WRITE(13,*)'Error opening file ninval.'
    STOP
  ENDIF
  ...
  DO INDEX=1,3
    ...
    DO
      ...
      DO I=NSTAR,NM,INM
        ...
        CALL CRSCT(E,COEFF)
        PRINT *,'Done calling CRSCT of energy ',E

        !Write out the cross sections as they are calculated, in order
        !to allow any error messages to be correlated with the energy
        !involved.  Note that the cross sections are not being saved,
      ...
      IF(MAXVAL(EST)<=ACCURACY.OR.NM==MAXE)THEN
        IF(MAXVAL(EST)>ACCURACY)THEN
          WRITE(13,*)' Convergence failure in region ',INDEX,&
            &' restricts the accuracy to ',MAXVAL(EST)
...
SUBROUTINE CRSCT(E,X1)
  ...
  IF(E>=ED.AND.E<=EC)THEN
    ...
    DO I=1,INOO
      ...
      ELSE
        ...
        !It is not a mistake to have BXX in the next line.
        RO(I)=FIT(NOO(2,I)-NOO(1,I)+1,EXX,BXX,CBR,E)
      ENDIF
      IF(RO(I)<ZERO)THEN
        WRITE(13,*)'Unusual behavior at point 1 in CRSECT.'
        WRITE(13,*)'You need to change the potential at large '
        WRITE(13,*)'separations, increase the value for the minimum'
        WRITE(13,*)'energy, or lower the accuracy in PC.in.'
        STOP
      ENDIF
    ENDDO
  ...
  IF(IPR/=0)THEN
    WRITE(13,*)' Convergence failure with energy ',EE,' restricts the &
      &accuracy to ',MAXVAL(ABS(ONE-X3/X2))
    PRINT *,' Convergence failure with energy ',EE,' restricts the &
      &accuracy to ',MAXVAL(ABS(ONE-X3/X2))
  ENDIF
  X1=X2
...
SUBROUTINE QINT(Y,FUN,LMAX)
  ...
  IF(EE>=ED.AND.EE<=EC)THEN
    ...
    IF(Y<ONE)THEN
      DO I=1,NAITK-1
        ...
        B2=ONE-VF(R1)/EE
        IF(B2<ZERO)THEN
          WRITE(13,*)'Error at position 1 in QINT.'
          WRITE(13,*)'You need to change the number of potential values'
          WRITE(13,*)'at large separations, increase the value for the'
          WRITE(13,*)'minimum energy, or lower the accuracy in PC.in.'
          STOP
        ENDIF
        B2=R1*SQRT(B2)
    ...
    IF(Y>-ONE)THEN
      DO I=1,NAITK-1
        ...
        B2=ONE-VF(R2)/EE
        IF(B2<ZERO)THEN
          WRITE(13,*)'Error at position 2 in QINT.'
          WRITE(13,*)'You need to change the number of potential values'
          WRITE(13,*)'at large separations, increase the value for the'
          WRITE(13,*)'minimum energy, or lower the accuracy in PC.in.'
          STOP
  ...
  ELSEIF(EE>ED.AND.EE<=EC2)THEN
    ...
    ELSE
      ...
      B2=ONE-VF(R4)/EE
      IF(B2<ZERO)THEN
        WRITE(13,*)'Error at position 4 in QINT.'
        WRITE(13,*)'You need to change the number of potential values'
        WRITE(13,*)'at large separations, increase the value for the'
        WRITE(13,*)'minimum energy, or lower the accuracy in PC.in.'
        STOP
      ...
      B2=ONE-VF(R5)/EE
      IF(B2<ZERO)THEN
        WRITE(13,*)'Error at position 5 in QINT.'
        WRITE(13,*)'You need to change the number of potential values'
        WRITE(13,*)'at large separations, increase the value for the'
        WRITE(13,*)'minimum energy, or lower the accuracy in PC.in.'
        STOP
...
SUBROUTINE ANGLE(E,B,THETA)
  ...
  REAL(SELECTED_REAL_KIND(18))::FA,FB,R,RBAR,VD
  ...
  IF(B<=ZERO)THEN
    WRITE(13,*)'Error at top of ANGLE, where B=',B
    WRITE(13,*)'You probably need to increase the minimum energy'
    WRITE(13,*)'in file PC.in.'
    STOP
  ENDIF
  ...
  IF(CASE1)THEN
    ...
    !Debug: save turning point for printing after THETA is computed


    !Trap errors and print an explanation.
    EA=ONE-R**3*VD(R)/TWO/B**2/E
    IF(EA(1)<=ZERO)THEN
      WRITE(13,*)'Error at point 1 in ANGLE, with R=',R
      WRITE(13,*)'You need to change the potential at large '
      WRITE(13,*)'separations, increase the value for the minimum'
      WRITE(13,*)'energy, or lower the accuracy in PC.in.'
      STOP
  ...
  !Debug: print turning point and theta for specific E and B
  IF(CASE1 .AND. ABS(E-1.0D-9)<1.0D-12 .AND. ABS(B-227.05D0)<1.0D0)THEN
    WRITE(*,*) 'DEBUG: E=',E,' B=',B
    WRITE(*,*) '  R=',R,' THETA=',THETA,' ROMAX=',ROMAX
  ENDIF
  ...
  ELSE
    ...
    EA=ONE
    IF(B<=ZERO)THEN
      WRITE(13,*)'Error at point 2 in ANGLE.'
      WRITE(13,*)'You need to change the potential at large '
      WRITE(13,*)'separations, increase the value for the minimum'
      WRITE(13,*)'energy, or lower the accuracy in PC.in.'
      STOP
    ELSE
      ...
      IF(EB(1)<ZERO)THEN
        ...
        IF(EB(1)<=ZERO)THEN
          WRITE(13,*)'Error at point 3 in ANGLE.'
          WRITE(13,*)'You need to change the potential at large '
          WRITE(13,*)'separations, increase the value for the minimum'
          WRITE(13,*)'energy, or lower the accuracy in PC.in.'
          STOP
        ENDIF
      ENDIF
      EB=ONE-B/RBAR-ACOS(R/RBAR)/SQRT(EB)
    ENDIF
...
SUBROUTINE GB(Y,FUNC,LMAX)
  ...
  REAL(SELECTED_REAL_KIND(18))::VF,R,Z1,Z2,Z3
  ...
  ELSE
    WRITE(13,*)'Zero value at point 1 in subroutine GB.'
    STOP
  ENDIF
...
SUBROUTINE MINIM(E,B,R)
  ...
  REAL(SELECTED_REAL_KIND(18))::Y,VF,VD,RSMAL,SCALE,RA,YA,RB,YB,&
    RT,YT,SIGN
  ...
  DO
    ...
    IF(R<0.5*POTEN(1,1))THEN
      WRITE(13,*)'R is below ',POTEN(1,1),' in subroutine MINIM.'
      STOP
    ENDIF
  ENDDO
  ...
  DO
    ...
    ENDIF
    IF(R>1.0D12)THEN
      WRITE(13,*)'R exceeded ',1.0D12,' in subroutine MINIM.'
      STOP
    ENDIF
  ENDDO
  ...
  IF(YA>ZERO)THEN
    IF(YB>ZERO)THEN
      WRITE(13,*)'There is a logic error in subroutine MINIM.'
      STOP
    ENDIF
    RT=RA
    YT=YA
...
SUBROUTINE AITKE(X,Y,XX,YY,N)
  ...
  IF(N<=1)THEN
    WRITE(13,*)' Error!  AITKE called with N=',N
    STOP
  ENDIF
...
REAL(SELECTED_REAL_KIND(18)) FUNCTION FIT(N,X,Y,Y2,XBAR)
  ...
  IF(N<=1)THEN
    WRITE(13,*)'Subroutine FIT was called with N=',N
    STOP
  ENDIF
...
REAL(SELECTED_REAL_KIND(18)) FUNCTION FIT2(N,X,Y,Y2,XBAR)
  ...
  !Check for the case N=1.
  IF(N<=1)THEN
    WRITE(13,*)'Subroutine FIT2 was called with N=',N
    STOP
  ENDIF
...
REAL(SELECTED_REAL_KIND(18)) FUNCTION FIT3(N,X,Y,Y2,XBAR)
  ...
  !Check for the case N=1.
  IF(N<=1)THEN
    WRITE(13,*)'Subroutine FIT3 was called with N=',N
    STOP
  ENDIF
